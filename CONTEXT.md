...
**Listing**: The S3 ListObjectsV2 surface — a metadata-only path backed by a live upstream directory walk. Never enters the object cache (no flights, no staging). _Avoid_: cache inventory (the list shows upstream state, not cache state).
**SigV4 gate**: Optional inbound verification at the business seam — signed requests (header or presigned form) are verified, unsigned requests pass through anonymous-first. Failure is a 403 that is never cached. _Avoid_: mandatory auth (the model is verify-if-present).
**Nocache profile**: A built-in fill policy where every request water-pipes origin-to-viewer with zero cache-object disk writes — the node streams, the CDN caches. Only the metadata database exists on disk. _Avoid_: passthrough profile (that one still stages segments on disk).
