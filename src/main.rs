...
async fn main() -> anyhow::Result<()> {
    ...
    // Both admission checks run BEFORE anything opens the database or binds a
    // port. They used to sit later in startup, which meant a config naming an
    // unset variable still opened the metadata store first -- on a node whose
    // cache_dir is live, a mis-typed env name could therefore reach (and, on a
    // lock conflict, quarantine) the production database before the process
    // died. A misconfigured node now fails without side effects.
    //
