...
# `Restart=always` restarts on its own, and systemd marks a requested stop
# with SERVICE_RESULT=success -- reporting those would turn each deploy into
# an outage alert on the far side.
send_down() {
