...
- `backend`: per-upstream concurrency is **two** semaphores since the 2026-09-12 review (ADR-0004): a metadata gate (`stat` / HEAD / list / link lookups) and a stream gate (cold-miss pumps, passthrough staging, staged reads). Both start at `concurrency_per_upstream`. The split exists because a single shared gate let a long transfer starve metadata: a HEAD measured 22 ms idle and 14.2 s while three cold pulls held the permits. Backoff: `Retry-After` honored exactly, else jittered exponential capped ~30 s (R3).
